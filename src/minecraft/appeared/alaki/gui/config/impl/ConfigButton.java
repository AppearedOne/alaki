package appeared.alaki.gui.config.impl;
...
public class ConfigButton {
    ...
    public double drawButton(double x, double y,int mouseX, int mouseY){
        ...
        RenderUtil.drawImage((int) (x + width - 12), (int) y + 1, 9, 9,new ResourceLocation("meth/icons/save.png"));
        ...
        RenderUtil.drawImage((int) (x + width - 12), (int) ((int) y + height - 10), 9, 9,new ResourceLocation("meth/icons/trash.png"));
    ...
    public boolean buttonClick(int mouseX, int mouseY, int state){
        if(state == 0){
            if(RenderUtil.isHovered(mouseX,mouseY,x,y, x+ width - 15,y+height)){
                ...
                Alaki.configManager.loadConfig(config.getName(), parent.keepKeybinds);
            }

            if(RenderUtil.isHovered(mouseX,mouseY, x + width - 12, y + height - 10, x + width - 12 + 9, y + height - 1)) {
                Alaki.configManager.removeConfig(config.getName());

                parent.reloadConfigs();
            }else if(RenderUtil.isHovered(mouseX,mouseY,x + width - 12, y + 1, x + width - 12 + 9, y + 10)){
                Alaki.configManager.saveConfig(config.getName());
