...
import appeared.alaki.utils.chat.NameUtil;
...
public final class GuiAltManager extends GuiScreen {
    ...
    public GuiAltManager(GuiScreen previousScreen) {
        ...
        try {
        	...
        	RenderUtil.drawImage(0, 0,this.width,this.height,new ResourceLocation("alaki/images/backg2.png"),1);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void drawScreen(int x, int y2, float z) {
        ...
        if (!generated) { Fonts.Arial18.drawCenteredStringWithShadow("Logged in as §1" + mc.getSession().getUsername(), (int) (width / 2F), 39, -7829368);}
    ...
    private void addButtons(int y) {
        ...
        buttons.add(new MainMenuButton(3,"Cracked", 15 + number, y, number,40));
        buttons.add(new MainMenuButton(2,"Clipboard", 20 + number * 2, y, number,40));
        buttons.add(new MainMenuButton(1,"Back", 25 + number * 3, y, number,40));
    }
