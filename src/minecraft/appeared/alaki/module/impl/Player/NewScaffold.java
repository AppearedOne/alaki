...
import appeared.alaki.events.impl.EventSafewalk;
...
import appeared.alaki.module.impl.Player.NewScaffoldUtil.*;
...
public class NewScaffold extends Module {
    ...
    private BooleanSetting safewalk = new BooleanSetting("EdgeCheck", false);
    private BooleanSetting blockFly = new BooleanSetting("BlockFly", false);
    private BooleanSetting tower = new BooleanSetting("Tower", false);
    private BooleanSetting sorting = new BooleanSetting("Sorting", false);
    private int stage;
    private BooleanSetting keeprots = new BooleanSetting("KeepRots", false);
    ...
    private BooleanSetting text = new BooleanSetting("Display HUD", true);
    private BooleanSetting keepY = new BooleanSetting("KeepY", true);
    private BooleanSetting auraCheck = new BooleanSetting("Check Aura", false);
    private BooleanSetting expand = new BooleanSetting("Expand", false);
    public ModeSetting scaffoldMode = new ModeSetting("Mode", "Hypixel", "AAC");
    public ModeSetting towermode = new ModeSetting("Tower Mode", "Hypixel", "Hypixel2", "Cubecraft", "Packet");
    private BooleanSetting boost = new BooleanSetting("Boost", false);
    public ModeSetting boostMode = new ModeSetting("Boost Mode", "Hypixel", "Hypixel2");
    private ModeSetting textMode = new ModeSetting("TextMode", "Normal", "None");
    private ModeSetting rotsMode = new ModeSetting("Rotations", "NCP", "Normal", "Hypixel", "Hypixel2", "Random");
    public NumberSetting expandamount = new NumberSetting("Expand Amount", 1, 1, 0, 15);
    public BooleanSetting slotChange = new BooleanSetting("SlotChange", true);
    public BooleanSetting edgecheck = new BooleanSetting("Safewalk", false);
    public BooleanSetting Jump = new BooleanSetting("Jump", false);

    float yaw = 0;
    float pitch = 0;

    public NumberSetting delay = new NumberSetting("Place Delay", 0, 1, 0, 300);
    public NumberSetting turn = new NumberSetting("Turn amount", 5, 1, 0 ,30); //HYPIXEL MODE ONLY
    ...
    public NumberSetting timerSpeed = new NumberSetting("Timer", 1, 0.1, 1, 10);
    public BooleanSetting eagle = new BooleanSetting("Eagle", false);
    public BooleanSetting raycast = new BooleanSetting("Raycast", false);
    public BooleanSetting sprintBypass = new BooleanSetting("SprintBypass", false); //Not useful
    private int slot;
    ...
    public NewScaffold() {
        super("NewScaffold", "Place blocks at your feet", Category.PLAYER, ServerType.All);
        this.addSettings(scaffoldMode, keepY, expand, blockFly, tower, sorting, keeprots, towermove, swing, text, auraCheck, towermode, boost, boostMode, textMode, rotsMode, expandamount, slotChange, edgecheck, delay, turn, keepSprint, timerSpeed, eagle,raycast, sprintBypass, Jump);
        ...
        if(event.getType() == EventType.Pre) {
            if (auraCheck.getValue()){
                if(Alaki.getInstance().getModuleManager().getModuleByName("Aura").isToggled()){
                	...
                	Alaki.getInstance().addNotification(new Notification("Disabled Aura", "Disabled due to Scaffold", 3000, NotificationType.WARNING));		
                }
            }

            if(scaffoldMode.getMode() == ("Hypixel") || scaffoldMode.getMode() == ("Cubecraft")) {
                int slot = this.getSlot();
                this.stopWalk = (getBlockCount() == 0 || slot == -1) && safewalk.getValue();
                ...
                if(scaffoldMode.getMode() == ("Hypixel")) {
                    if (keepSprint.getValue()) {
                        if (sprintBypass.getValue()) {
                        }
                    }
                }
                ...
                if (mc.gameSettings.keyBindJump.isKeyDown() && tower.getValue() && (this.towermove.getValue() || !mc.thePlayer.isPotionActive(Potion.jump))) {
                    switch(towermode.getModes().toString()) {
                        case "Hypixel2":
                ...
                if (this.isPlaceTick) {
                    ...
                    Rotation targetRotation = new Rotation(appeared.alaki.module.impl.Player.NewScaffoldUtil.setBlockAndFacing.BlockUtil.getDirectionToBlock(blockData.getPosition().getX(), blockData.getPosition().getY(), blockData.getPosition().getZ(), blockData.getFacing())[0], 81f);
                    Rotation limitedRotation = appeared.alaki.module.impl.Player.NewScaffoldUtil.setBlockAndFacing.BlockUtil.limitAngleChange(new Rotation(yaw, event.getRotationPitch()), targetRotation, this.rotsMode.equals("Hypixel") ? (float) ThreadLocalRandom.current().nextDouble(20, 30) : (int) this.turn.getValue());
                    ...
                    if (this.blockData.getFacing().getName().equals("west")) {
                        targetRotationYaw = -90;
                    }
                    if (this.blockData.getFacing().getName().equals("north")) {
                        targetRotationYaw = 0;
                    }
                    if (this.blockData.getFacing().getName().equals("east")) {
                        targetRotationYaw = 90;
                    }
                    if (this.blockData.getFacing().getName().equals("south")) {
                        targetRotationYaw = 180;
                    }
                    float speed = (float) ThreadLocalRandom.current().nextDouble(1.5, 2.5);
                    float yawDifference = event.getLastYaw() - targetRotationYaw;
                    if(yawDifference < 91) {
                        yaw = event.getLastYaw() - (yawDifference / speed);
                    }else {
                        yaw = targetRotationYaw;
                    }
                    ...
                    if(this.rotsMode.equals("NCP") || this.rotsMode.equals("Normal")) {
                        event.setPitch(85);
                        event.setYaw(yaw);
                    }else if(this.rotsMode.equals("Hypixel")){
                        event.setYaw(this.yaw);
                        event.setPitch(81f); //79.44f maybe?
                    }else if(this.rotsMode.equals("Hypixel2")){
                        event.setYaw(this.yaw); //no random + less turn
                        event.setPitch(81f);
                    }else if(this.rotsMode.equals("Random")){
                    ...
                    if(this.boost.getValue()) {
                        if(this.boostMode.equals("Mineplex")) {
                            if (stage > 10)
                                stage = 0;
                            stage++;
                ...
                if (mc.theWorld.getBlockState(pos).getBlock() instanceof BlockAir) {
                    ...
                    if (currentPos != null) {
                        float facing[] = appeared.alaki.module.impl.Player.NewScaffoldUtil.setBlockAndFacing.BlockUtil.getDirectionToBlock(currentPos.getX(), currentPos.getY(), currentPos.getZ(), currentFacing);
                        ...
                        this.rotated = !(this.raycast.getValue() && !rayTrace(yaw, pitch));
                        if(this.rotsMode.equals("NCP") || this.rotsMode.equals("Normal")) {
                            event.setYaw(yaw);
                ...
                if(this.rotsMode.equals("NCP") || this.rotsMode.equals("Normal")) {
                    mc.thePlayer.rotationYawHead = event.getRotationYaw();
                    mc.thePlayer.rotationPitchHead = event.getRotationPitch();
                    mc.thePlayer.renderYawOffset = event.getRotationYaw();
                }else if(this.rotsMode.equals("Hypixel")){
                    mc.thePlayer.setRotationYawHead(this.yaw);
                    mc.thePlayer.renderYawOffset = this.yaw;
                    mc.thePlayer.rotationPitchHead = 81;
    ...
    /*public final void onPacket(PacketEvent e) {
        ...
        if(this.scaffoldMode.equals("Hypixel")) {
            if (keepSprint.getValue()) {
                if (sprintBypass.getValue()) {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if(event.getType() == EventType.Post) {
            switch(scaffoldMode.getModes().toString()) {
            
                case "Hypixel":
                    if(this.slotChange.getValue()) {
                        int slot = this.getSlot();
                        if (slot != -1 && this.blockData != null) {
                            ...
                            //   Artemis.addClientChatMessage(currentSlot + "a a " + Math.random());
                        }
                    }else {
                        int slot = this.getSlot();
                        if (slot != -1 && this.blockData != null) {
                ...
                case "AAC":
                    ...
                    if (currentPos != null) {
                        if (timer.hasTimePassed((long) this.delay.getValue()) && rotated) {
    ...
    public void onSafe(EventSafewalk event) {
        boolean downwards = (Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode()) && !mc.gameSettings.keyBindJump.isKeyDown() && blockFly.getValue() && mc.thePlayer.onGround);
        if (mc.thePlayer.onGround && this.safewalk.getValue() && !downwards) {
            event.setSafe(true);
        }
    }
    ...
    public void onEnable() {
        super.onEnable();
        mc.timer.timerSpeed = timerSpeed.getFloatValue();
        stage = 0;
    ...
    public void onDisable() {
        ...
        mc.timer.timerSpeed = 1.0f;
        if(scaffoldMode.equals("Hypixel")) {
            if(this.slotChange.getValue()) {
                mc.thePlayer.inventory.currentItem = slot;
            }
            if(UpdateEvent != null) {
                ...
                //   Rotation limitedRotation = appeared.meth.module.impl.Player.NewScaffoldUtil.setBlockAndFacing.BlockUtil.limitAngleChange(new Rotation(yaw, UpdateEvent.getPitch()), targetRotation, (float) ThreadLocalRandom.current().nextDouble(20, 30));
                // yaw = limitedRotation.getRotationYaw();
                /// pitch = limitedRotation.getPitch();
                //  UpdateEvent.setYaw(UpdateEvent.getRotationYaw());
                // UpdateEvent.setPitch(90);
            }
        }
    ...
    private boolean getPlaceBlock(final BlockPos pos, final EnumFacing facing) {
        ...
        if((edgecheck.getValue() ? isOnEdgeWithOffset(0.15) : timer.hasTimePassed((long) delay.getValue()))) {
    ...
    private BlockData getBlockData() {
        ...
        if(mc.thePlayer.isMoving()){
            if(this.expand.getValue()) {
                playerpos = expand.offset(mc.thePlayer.getHorizontalFacing(), (int) this.expandamount.getValue());
            }
        }
        boolean tower = !this.towermove.getValue() && this.tower.getValue() && !mc.thePlayer.isMoving();
        if (!this.blockFly.getValue() && this.keepY.getValue() && !tower) {
            if(this.expand.getValue()) {
                playerpos = new BlockPos(new Vec3(mc.thePlayer.getPositionVector().xCoord, this.startY, mc.thePlayer.getPositionVector().zCoord)).offset(mc.thePlayer.getHorizontalFacing(), (int) this.expandamount.getValue());
            }else {
    ...
    private int getSlot() {
        for (int k = 0; k < 9; ++k) {
            ...
            if (itemStack != null && this.isValid(itemStack) && (this.sorting.getValue() ? (this.getBestStack() == itemStack) : itemStack.stackSize >= 1)) {
                return k;
            }
        }
        return -1;
    ...
    public void onRender2D(OverlayEvent event) {
        ScaledResolution sr = new ScaledResolution(Minecraft.getMinecraft());
        int blocks = getBlockCount();
        String text = (new StringBuilder(String.valueOf(blocks))).toString();
        if(this.text.getValue()) {
            if(textMode.getMode() == "Normal") {
                
                    //TODO draw string with blockCount
                	//Artemis.instance.rfrs.drawCenteredString("§" + this.getStackColorSuffix(blocks) + String.valueOf(text) + " §fBlocks", (sr.getScaledWidth() / 2) + 0.1F, (sr.getScaledHeight() / 2) + Artemis.instance.rfrs.getHeight(text) + 0.1F, -1);                 
    ...
    private void drawCircle(Entity entity, float partialTicks, double rad, double height, double alpha) {
        ...
        final float hue = (float) (ColorUtil.getColor(1));
        Color color = ColorUtil.getAstolfoColor(200000, 1);
